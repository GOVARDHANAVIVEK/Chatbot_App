...
function handleGreetings(){
    addMessage(greet_response,'bot-message')
    appointmentDetails = {};
    intent_tracking = "";
    greet_response=""
}
...
async function track_intent(utterance) {
    try {
        ...
        if(data.intent == "greeting_flow" || data.intent=='thanking_flow'){
            greet_response = data.Greetings
        }else{
...
function end_of_dialog() {
    ...
    const end_messages = [
        ...
        "Before we finish, do you need anything else?",
        "Can I provide you with more information?",
        "Do you have any other needs or questions?"
    ]
    
    setTimeout(() => {
        addMessage(getRandomPrompt(end_messages), 'bot-message');
    }, 1000);
}
