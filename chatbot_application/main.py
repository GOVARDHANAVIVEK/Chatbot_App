...
async def handle_initial_message(message:Message):
    ...
    try:
        ...
        print(intent)
        if(intent=="greeting_flow" or intent=='thanking_flow'):
            greet_response = data_item['greet']
            print(greet_response)
            print({"Greetings":greet_response,"intent": intent})
            return {"Greetings":greet_response,"intent": intent}
        else:
